/* eslint-disable no-unused-vars */
import LitWithoutShadowDom from "../../../base/LitWithoutShadowDom";
import { html } from "lit";
import input from "./input";
import inputimage from "./input-image";
import { msg, updateWhenLocaleChanges } from "@lit/localize";

class FormAddStoryApp extends LitWithoutShadowDom {
  constructor() {
    super();
    updateWhenLocaleChanges(this);
  }

  render() {
    return html`
      <h1 class="form-input-text">
        <span>${msg(`Share your Story here!`)}</span>
      </h1>
      <form class="form-input-form" novalidate>
        <input-component
          class="input-component-container input-component-root-class-name20"
          name="${msg(`Name`)}"
          type="text"
          placeholder="${msg(`Sulhan`)}"
          idscss="storyname-input"
          invalidFeedbackMessage="${msg(`Please complete this field`)}"
          required
        ></input-component>
        <input-component
          class="input-component-container input-component-root-class-name20"
          name="${msg(`Your Story`)}"
          type="text"
          placeholder="${msg(`Sulhan will definitely be successful, aamiin`)}"
          idscss="description-input"
          invalidFeedbackMessage="${msg(`Please complete this field`)}"
          required
        ></input-component>
        <inputimage-component
          class="input-component-container input-component-root-class-name20"
          inputId="validationCustomEvidence"
          invalidFeedbackMessage="${msg(`Please upload a photo`)}"
          required
        ></inputimage-component>
        <button class="btn form-input-hero-button1">
          <span>${msg(`Share my Story!`)}</span>
        </button>
      </form>
    `;
  }
}

customElements.define("formaddstory-component", FormAddStoryApp);
